    ...
    this.store
      ...
      .subscribe((sensor: SensorDetails | undefined) => {
        ...
        this.sensorDetailsForm = this.formBuilder.group({
          ...
          allow_group_admins_to_edit: [
            sensor?.allow_group_admins_to_edit,
            [Validators.required],
          ],
          active_alerts: [sensor?.active_alerts, [Validators.required]],
          high_temp_alert: [
            sensor?.high_temp_alert ? sensor?.high_temp_alert : 30,
          ],
          low_temp_alert: [
            sensor?.low_temp_alert ? sensor?.low_temp_alert : 15,
          ],
          high_humidity_alert: [
            sensor?.high_humidity_alert ? sensor?.high_humidity_alert : 80,
          ],
          low_humidity_alert: [
            sensor?.low_humidity_alert ? sensor?.low_humidity_alert : 20,
          ],
        });
